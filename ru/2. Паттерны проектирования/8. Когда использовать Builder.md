...
**Builder** — когда много параметров (4+), особенно опциональных.
**Factory Method** — когда есть несколько способов создать объект (fromJSON, fromXML, fromCSV).
**Constructor** — когда 1-2 обязательных параметра.
...
**Wither** — метод в функциональном программировании, возвращающий новый объект с изменённым полем (name = name.withX(x)).
**GC pressure** — частые аллокации вызывают частые GC-циклы.
**Step Builder (Staged Builder)** — компилятор не даёт пропустить шаг через интерфейсные типы.
...
**Связанные темы:**
- [В чём разница между Factory Method и Abstract Factory](7.%20В%20чём%20разница%20между%20Factory%20Method%20и%20Abstract%20Factory.md) — порождающие паттерны
- [Что такое Prototype pattern](9.%20Что%20такое%20Prototype%20pattern.md) — копирование объектов
- [Что такое Singleton](3.%20Что%20такое%20Singleton.md) — когда Builder нужен для сложного Singleton
