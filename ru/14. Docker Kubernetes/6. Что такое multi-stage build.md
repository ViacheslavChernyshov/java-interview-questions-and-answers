...
**Multi-stage build (Многоэтапная сборка)** — это способ создать маленький Docker-образ, используя несколько этапов в одном Dockerfile. На первом этапе вы собираете приложение, на втором — запускаете его. В итоговый образ попадает только готовый файл, а все инструменты сборки остаются «за бортом».
