...
Pod'ы **не постоянны**. Если Pod упал — он не восстанавливается, а заменяется новым. Новый Pod получает новый IP-адрес.
...
В 90% случаев используется правило "один контейнер на Pod". Но есть паттерны для нескольких контейнеров:
