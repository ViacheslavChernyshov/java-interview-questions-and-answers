...
public CompletableFuture<String> withRetry(Supplier<CompletableFuture<String>> operation, int maxRetries) {
    return operation.get()
        .exceptionally(ex -> {
            ...
            throw new RuntimeException("All retries failed", ex);
        });
}
...
.exceptionally(ex -> withRetry(operation, maxRetries).join());

// ✅ Уменьшаем
.exceptionally(ex -> withRetry(operation, maxRetries - 1).join());
```
