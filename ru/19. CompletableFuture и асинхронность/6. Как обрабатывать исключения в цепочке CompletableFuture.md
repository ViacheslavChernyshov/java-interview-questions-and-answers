...
1. **exceptionally не ловит исключения из thenApply:**
```java
// ❌ Исключение в thenApply не попадёт в downstream exceptionally
cf.thenApply(s -> {
    throw new RuntimeException("Error");  // поймаются
}).exceptionally(ex -> "default");  // ✅ OK
