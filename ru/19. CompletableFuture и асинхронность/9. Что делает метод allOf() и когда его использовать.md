...
public static CompletableFuture<Void> allOf(CompletableFuture<?>... cfs) {
    return new BiRelay<>(
        new AndTree(cfs, 0, cfs.length - 1),  // дерево зависимостей
        null, null, null, null  // no result
    );
}

// Использует дерево BiRelay для эффективного ожидания
// Когда все CF завершаются — вызывается postComplete
```
