...
| Phantom Read | **Нет** (в MySQL/InnoDB — особенность!) |
...
MySQL/InnoDB реализует Repeatable Read через комбинацию **MVCC** и **Next-Key Locking**:
...
1. **Statement-based Replication (историческая причина):** В старых версиях MySQL репликация копировала SQL-запросы. Для корректной репликации на слейвах требовалась детерминированность, которую обеспечивал Repeatable Read.
...
```
MySQL 5.7: AUTO-INC lock — table-level lock на время INSERT
MySQL 8.0: AUTO-INC lock — released сразу после генерации ID (interleaved inserts possible)
