...
**Ключевой момент:** Propagation работает **только при вызове через Spring Proxy**. Вызов `this.method()` внутри того же класса bypass-ит прокси.
...
- REQUIRED + 1 REQUIRES_NEW: ~20,000 TPS (suspend/resume overhead + connection contention)
