...
В обоих случаях proxy оборачивает target object. Внешние вызовы идут через proxy → interceptor → target. Внутренние вызовы (`this.method()`) идут напрямую: target → target.
