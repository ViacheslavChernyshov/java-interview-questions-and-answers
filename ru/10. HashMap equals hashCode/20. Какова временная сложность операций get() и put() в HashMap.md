...
Неважно, 10 элементов в карте или 10 000 — поиск занимает примерно одинаковое время.
...
В Java 8+ O(n) сохраняется только в двух случаях:

1. **capacity < 64** — treeification не срабатывает, при 8+ коллизиях = O(n)
2. **Iteration** — O(capacity + size), при huge capacity = O(n)
