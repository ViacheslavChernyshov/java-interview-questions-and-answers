...
Компилятор Java старается сделать конкатенацию строк как можно быстрее. Если он видит, что строки известны заранее, он склеивает их ещё до запуска программы.
...
- Для форматирования: `String.format()` медленнее (regex-парсинг паттерна). Рассмотрите текстовые блоки (Java 15+) или специализированные библиотеки
