...
**Простая аналогия:** Кодировка — как язык перевода. Если вы перевели фразу на французский (encode), а переводчик на другой стороне ожидает немецкий (decode) — получится бессмыслица.
...
```
Проблема: один из продюсеров использовал `getBytes()` без кодировки (дефолт на Windows = Cp1252). Consumer на Linux читал как UTF-8 → corrupted messages. Fix: единый стандарт UTF-8 на уровне Kafka-контракта.
