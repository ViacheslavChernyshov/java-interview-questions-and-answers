...
1. **Ошибка:** Думать, что GC автоматически почистит пул
   **Решение:** Если есть strong reference — строка не будет удалена
...
2. **GC и String Pool cleaning:** В Java 8+ JVM может удалять unreachable entries из StringTable во время Full GC. Но это работает только если на строки нет strong references.
