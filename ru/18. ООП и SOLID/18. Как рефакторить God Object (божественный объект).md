...
Метрика **LCOM (Lack of Cohesion in Methods)** формализует проблему:
...
| **Транзакционная целостность:** God Object обеспечивал атомарность через `synchronized` | После разделения нужны координатор транзакций или Saga-паттерн |
...
| **Big Ball of Mud:** Всё связано со всем, нет чётких границ | Начните с тестов — если метод не получается протестировать без 20 моков, это точка разреза |
...
- CPU instruction cache利用率 выше — код маленьких классов помещается в L1 cache (32 КБ).
...
1. **Неделя 1:** Написали characterization tests на каждый публичный метод (87 тестов).
...
**IntelliJ IDEA** — «Metrics Reloaded» плагин: показывает LCOM,耦合度, глубину наследования прямо в редакторе.
...
7. **Bounded Contexts (DDD)** — режьте по бизнес-границам, не по техническим. `UserService`, `PaymentService`, `NotificationService` — это bounded contexts.
