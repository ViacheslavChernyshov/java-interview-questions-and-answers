...
- Всегда при проектировании новых классов
...
**Связь**: Соблюдение SRP всегда ведёт к высокой Cohesion. Класс с одной ответственностью естественно имеет методы, работающие над одной задачей.

### Уровни Cohesion по Майерсу
...
### Метрика LCOM (Lack of Cohesion in Methods)

LCOM анализирует, сколько методов используют общие поля:
- Если 10 методов: 5 работают с полем A, 5 с полем B → LCOM высокий → разделить на 2 класса
...
|--------|----------|----------|-----------|
| **God Object** | Низкая | Низкий | ❌ Хрупкий |
| **Атомарная пыль** | Высокая | Чудовищный | ❌ Сложный |
...
| Package-by-Layer → низкая связность | Package-by-Feature |
...
**LCOM4** вычисляется через граф зависимостей:
- Узлы = методы + поля
...
1. **Anemic Domain Model**: Классы только с getter/setter → LCOM = 0, но это не значит "хорошо"
   - **Решение**: DDD — логика рядом с данными
