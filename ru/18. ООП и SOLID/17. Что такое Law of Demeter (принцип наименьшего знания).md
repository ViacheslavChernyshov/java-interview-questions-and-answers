...
| **Train Wreck (цепочки)** | Меньше кода, «быстро» написать | Хрупкость: изменение в `Address` ломает 10 классов, сложные моки |
...
- Проверяет vtable (виртуальную таблицу методов).
...
**Делегирование** сокращает цепочку до одного вызова, который JIT может заинлайнить полностью:
```java
...
**SonarQube** — правило `S2259` (null pointer dereference) часто выявляет train wreck. Плагин **SonarJava** имеет метрику «Cyclomatic Complexity of Call Chains».

**Structure101 / jQAssistant** — визуализируют «звёзды» связности. Train wreck цепочки отображаются как длинные рёбра в графе зависимостей.
