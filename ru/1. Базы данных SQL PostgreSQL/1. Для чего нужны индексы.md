...
**Основная идея:**
- Без индекса база данных просматривает всю таблицу подряд (Seq Scan)
...
Индекс хранит значения колонок в **отсортированном виде** и указатели (TID — Tuple ID) на физические строки в таблице. Самый распространённый тип индекса — **B-Tree** (создаётся по умолчанию).
...
- Маленькие таблицы (< 1000 строк) — Seq Scan быстрее
