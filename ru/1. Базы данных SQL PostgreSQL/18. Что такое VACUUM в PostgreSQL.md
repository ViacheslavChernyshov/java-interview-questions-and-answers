...
**Важно:**
- Обычный VACUUM **в основном не возвращает место** операционной системе из середины файла (освобождает блоки внутри файла для новых строк)
- В PostgreSQL 12+ обычный VACUUM может вернуть место ОС с **конца файла** (`vacuum_truncate`), если хвостовые страницы оказались полностью пустыми
- `VACUUM FULL` полностью перестраивает файл и возвращает место ОС, но **блокирует таблицу** (AccessExclusiveLock)
...
**Связанные темы:**
- [Как работает MVCC в PostgreSQL](17.%20Как%20работает%20MVCC%20в%20PostgreSQL.md) → dead tuples, Hint Bits, XID Wraparound
- [Зачем нужен ANALYZE](19.%20Зачем%20нужен%20ANALYZE.md) → Autovacuum запускает ANALYZE
- [Как оптимизировать медленные запросы](21.%20Как%20оптимизировать%20медленные%20запросы.md) → Bloat влияет на производительность
