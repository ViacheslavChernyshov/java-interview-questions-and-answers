...
8. **pgBouncer** рекомендован при > 50 подключений. Для малых проектов
   (< 10 пользователей) можно обойтись без него.
...
- **Write Amplification** (усиление записи): каждый INSERT/UPDATE записывает
  не только в таблицу, но и во ВСЕ индексы. 10 индексов = запись × 10.
  Физически записывается в 10x больше данных, чем изменил пользователь.
...
**Связанные темы:**
- [Для чего нужны индексы](1.%20Для%20чего%20нужны%20индексы.md) → первое решение для оптимизации
- [Что такое explain plan](20.%20Что%20такое%20explain%20plan.md) → EXPLAIN (ANALYZE, BUFFERS)
- [Зачем нужен ANALYZE](19.%20Зачем%20нужен%20ANALYZE.md) → статистика для планировщика
- [Как работает MVCC в PostgreSQL](17.%20Как%20работает%20MVCC%20в%20PostgreSQL.md) → Bloat и длинные транзакции
