...
public record User(String name) {
    // ❌ Нельзя переопределить аксессор
    public String name() { return name.toUpperCase(); }  // error
    
    // ✅ Но можно добавить метод с другим именем
    public String upperName() { return name.toUpperCase(); }
    
    // ❌ Нельзя переопределить final методы из java.lang.Record
    // equals, hashCode, toString — можно, но осторожно
}
```
