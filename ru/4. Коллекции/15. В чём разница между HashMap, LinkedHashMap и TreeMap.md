...
| **Скорость** | Быстрая | Чуть медленнее | Медленнее |
...
```
HashMap: массив корзин → лучше локальность
LinkedHashMap: + DLL → итерация по списку
TreeMap: узлы дерева → worst locality

→ Итерация: HashMap > LinkedHashMap > TreeMap
```
