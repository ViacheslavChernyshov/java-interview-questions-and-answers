...
### Резюме для Senior
- `record` — специальная языковая конструкция для неизменяемых классов-значений.
Это не просто sugar — record имеет собственные ограничения (нельзя наследовать,
нельзя иметь instance-поля кроме declared).
...
- [Как реализовать Builder pattern для иммутабельного класса](26.%20Как%20реализовать%20Builder%20pattern%20для%20иммутабельного%20класса.md)
