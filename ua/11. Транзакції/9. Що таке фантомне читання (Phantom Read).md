...
Коли ви виконуєте `SELECT WHERE status = 'pending'`, база повертає рядки, наявні на момент знімка. Але якщо інша транзакція вставляє новий рядок з `status = 'pending'`, ваш повторний запит побачить його як "фантом".
