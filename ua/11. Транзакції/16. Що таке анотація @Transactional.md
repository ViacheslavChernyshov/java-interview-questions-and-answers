...
Саме тому `@Transactional` **не працює між потоками** при стандартній конфігурації — ThreadLocal не передається. `@Async` методи отримують окремий transaction context. При використанні кастомних TaskExecutor з ThreadLocal propagation або Virtual Threads (Java 21+) можливе прокидання контексту, але це вимагає явного налаштування.
