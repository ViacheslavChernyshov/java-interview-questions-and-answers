...
```
Якщо JIT довів, що об'єкт не "втікає" з потоку — синхронізація видаляється. Але:
...
- **Що робить JVM для оптимізації `StringBuffer`?** — Lock Elision через Escape Analysis: якщо об'єкт не "втікає" з потоку — JIT може прибрати `synchronized`. Але не гарантовано.
