...
Якщо `action` сам викине виняток — НОВЕ виняток замінить оригінальне як результат стадії. Оригінальне буде втрачено (не додається як suppressed).

```java
cf.whenComplete((result, ex) -> {
    throw new RuntimeException("New error");  // замінить оригінальний ex!
});
// Оригінальне виняток втрачено!
```
...
- **whenComplete викине — що буде?** — Нове виняток замінить оригінальне, оригінальне втрачено
