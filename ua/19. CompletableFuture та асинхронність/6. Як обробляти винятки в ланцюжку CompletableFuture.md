# Як обробляти винятки в ланцюжку CompletableFuture

## 🟢 Junior Level

У CompletableFuture є **3 основні способи** обробки винятків:
...
1. **exceptionally ЛОВИТЬ виняток з thenApply:**
```java
// ✅ виняток в thenApply спіймається в downstream exceptionally
cf.thenApply(s -> {
    throw new RuntimeException("Error");
...
```java
// виняток загортаються в CompletionException
cf.exceptionally(ex -> {
    // ex — CompletionException, не оригінальне виняток!
...
```java
cf.exceptionally(ex -> {
    throw new RuntimeException("Handler error");  // Нове виняток
}).exceptionally(ex -> {
    // Спіймає виняток з першого handler
    return "recovered";
});
```
...
// ❌ Створення нових винятків без причини
```
...
- виняток загортаються в CompletionException, оригінал через getCause()
- exceptionallyCompose (Java 12+) — повернення іншого CF при помилці (для retry)

**Часті уточнюючі питання:**
- **whenComplete перехопить виняток?** — Ні, тільки спостерігає. виняток залишається в CF
...
- «whenComplete ловить виняток» — він тільки спостерігає, не перехоплює
- «exceptionally викликається завжди» — тільки при помилці, handle викликається завжди
- «виняток в handler не обробиться» — handler виняток теж ловиться downstream exceptionally

**Пов'язані теми:**
- [[7. У чому різниця між handle(), exceptionally() і whenComplete()]]
- [[18. Що станеться, якщо в ланцюжку CompletableFuture виникне виняток]]
