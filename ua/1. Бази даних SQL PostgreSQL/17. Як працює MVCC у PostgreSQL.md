...
**MVCC (Multi-Version Concurrency Control)** — механізм, який дозволяє PostgreSQL показувати кожному користувачеві **свою версію даних**, не блокуючи читання при записі.
...
- Кожна транзакція бачить тільки ті рядки, які існували на момент її початку
...
| **xmax** | ID транзакції, яка видалила рядок (0 = не видалений) |
...
-- → Бачить рядок з xmin=101 (зафіксована)
-- → name='Petr'
```
...
```
Snapshot:
  xmin = 100  ← Усі транзакції < 100 зафіксовані
  xmax = 105  ← Усі транзакції >= 105 ще не почалися
  xip_list = [101, 102, 103]  ← Активні транзакції

Правило видимості:
  Рядок видимий, якщо:
    - його xmin < snapshot.xmin АБО xmin в xip_list (ще активна)
...
-- 3. Індекси оновлюються (посилання на нову ctid)
```
...
1. **Bloat (роздування)**
   ```sql
   -- Таблиця займає в 10 разів більше місця, ніж повинна
...
3. **Transaction ID Wraparound**
   ...
   -- > 1.5 млрд → база перейде в READ ONLY
   ```
...
**CLOG** — файли в `pg_xact/`, де кожна транзакція = 2 біти (IN_PROGRESS/COMMITTED/ABORTED). Це НЕ WAL. Читання CLOG = мікросекунди, але при мільйонах рядків → секунди.

**Рішення:** PostgreSQL використовує **Hint Bits** — 2 біти в заголовку рядка:
...
2. Проставляє Hint Bits у заголовок рядка
3. Усі наступні читачі перевіряють Hint Bits миттєво
...
-- У xmax записується посилання на pg_multixact
...
-- 2. Оновити ВСІ індекси (нова ctid)
...
-- Умови для HOT:
-- - Оновлювані колонки НЕ індексовані
...
-- hot_pct > 70% → чудово
-- hot_pct < 20% → проблема (занадто багато індексованих оновлень)
```
...
-- «Заморожування» = заміна xmin на FrozenXID, який ЗАВЖДИ «в минулому».
-- Заморожений рядок видимий будь-якій транзакції. Як дата «1900 рік» на
...
1. **Long-running transactions**
   ...
   -- → Майстер накопичує dead tuples → Bloat!
   ```

2. **TOAST та MVCC**
   ```sql
   -- Великі поля (text, bytea) зберігаються в TOAST-таблиці
   -- TOAST-рядки теж версіонуються
   -- Але TOAST не використовує MVCC для читання!
   -- → Може повернути дані видаленого рядка в рідкісних випадках
   ```
...
- autovacuum був відключений "для продуктивності"
- age(datfrozenxid) = 1.9 млрд (95%)
- База почала попереджати в логах
- Рішення: терміновий `VACUUM FREEZE` на всій БД
- Урок: ніколи не відключайте autovacuum!
...
1. **Не тримайте транзакції відкритими** довше за необхідність
2. **autovacuum — НЕ відключати!** Налаштовуйте, але не вимикайте
...
8. **VACUUM FULL** тільки в maintenance window (блокує!)
...
- **Visibility Map** критична для Index Only Scan
...
**Обов'язково знати:**
- MVCC: кожен рядок має xmin (створений) і xmax (видалений)
- Snapshot: транзакція бачить рядки, що існували на момент початку
- UPDATE = DELETE + INSERT (нова версія рядка, стара → dead tuple)
- Hint Bits: 2 біти в заголовку рядка → уникаємо читання CLOG після першого читача
...
- «Що буде, якщо відключити autovacuum?» → XID Wraparound → база перейде в READ ONLY
...
- ❌ «autovacuum можна відключити для продуктивності» (Wraparound → READ ONLY!)
- ❌ «MVCC блокує читання при записі» (читачі не блокують письменників!)
- ❌ «Hint Bits WAL-logged» (ні, після crash перераховуються)

**Пов'язані теми:**
- [Що таке VACUUM у PostgreSQL](18.%20Що%20таке%20VACUUM%20у%20PostgreSQL.md) → очищення dead tuples
- [Навіщо потрібен ANALYZE](19.%20Навіщо%20потрібен%20ANALYZE.md) → збір статистики
- [Для чого потрібні індекси](01.%20Для%20чого%20потрібні%20індекси.md) → HOT та Visibility Map
