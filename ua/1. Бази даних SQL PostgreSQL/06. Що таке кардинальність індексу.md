...
**Навіщо це знати:** планувальник PostgreSQL використовує кардинальність, щоб вирішити — використовувати індекс чи прочитати таблицю цілком. Висока кардинальність = багато унікальних значень = індекс ефективний. Низька = мало унікальних значень = скоріш за все, Seq Scan буде швидшим за індекс.

**Проста аналогія:** Уявіть мішок із кульками різних кольорів. Кардинальність — це скільки в мішку кульок **різних** кольорів (не рахуючи кількість кульок кожного кольору).
...
- Індекси ефективніші на полях із **високою** кардинальністю
...
SELECT COUNT(DISTINCT gender) FROM users; -- 2 (M і F)
...
**Коли це важливо:**
- При виборі полів для індексації
...
Ці поняття пов'язані, але відрізняються:
...
```sql
-- Подивитися статистику за колонками
SELECT 
...
**Поріг перемикання (приблизний):**

| % рядків у вибірці | План |
|---|---|
| < 1–5% | Index Scan |
| 5–20% | Bitmap Index Scan (збирає TID у бітову карту, потім читає heap — менше випадкових I/O, ніж Index Scan) |
| > 20% | Seq Scan |
...
**Чому різні плани:** PostgreSQL зберігає MCV (Most Common Values) — список найчастіших значень з їхньою частотою. Для 'ERROR' (1%) планувальник знає, що знайде ~10 000 рядків з 1 млн — Index Scan вигідний. Для 'ACTIVE' (95%) — 950 000 рядків, що дешевше прочитати Seq Scan (одне послідовне читання vs 950 000 випадкових).

PostgreSQL зберігає **Most Common Values (MCV)** і може обирати різні плани для різних значень!

### Типові помилки

1. **Індексування полів із низькою кардинальністю**
   ```sql
   -- ❌ Погано: всього 2 значення
   CREATE INDEX idx_users_gender ON users(gender);
   -- Seq Scan все одно буде обрано для більшості запитів
   ```

2. **Ігнорування складеної кардинальності**
   ```sql
...
2. **most_common_vals (MCV)**
   ```
   -- Топ найчастіших значень та їхня частота
   -- Приклад для status:
...
-- Для колонок із нерівномірним розподілом збільшуємо точність
...
1. **Кореляція вбиває індекс**
   ...
   -- ⚠️ Блокує таблицю! Робіть у maintenance window
   ```
...
| 100 (за замовчуванням) | Добра | Нормально |
| 1000 | Відмінна | У 10x довше |
| 10000 | Максимальна | У 100x довше |

**Memory Footprint статистики:**
- Кожен кошик гістограми: ~24 байти
...
- Проблема: Запити за ERROR були повільними
...
- E-commerce: замовлення за `(country, city)`
- Проблема: Планувальник помилявся в оцінці на 2 порядки
- Причина: city сильно залежить від country (Київ тільки в Україні)
- Рішення:
  ```sql
...
- statistics_target = 100 (за замовчуванням), 1000 — для skewed колонок (у 10x довше ANALYZE)
...
- ❌ «n_distinct = -1 означає немає унікальних значень» (навпаки — 100% унікальні!)
- ❌ «statistics_target не впливає на продуктивність» (впливає на час ANALYZE і точність планів)

**Пов'язані теми:**
- [Для чого потрібні індекси](01.%20Для%20чого%20потрібні%20індекси.md) → селективність і кардинальність
- [Навіщо потрібен ANALYZE](19.%20Навіщо%20потрібен%20ANALYZE.md) → збір статистики, n_distinct, MCV, histogram
- [Що таке складений індекс](03.%20Що%20таке%20складений%20індекс.md) → складена кардинальність
- [Як оптимізувати повільні запити](21.%20Як%20оптимізувати%20повільні%20запити.md) → вплив статистики на план
