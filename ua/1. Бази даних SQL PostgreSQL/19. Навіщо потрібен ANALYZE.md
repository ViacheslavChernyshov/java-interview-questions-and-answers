...
**ANALYZE** — команда, яка збирає статистику про дані в таблиці, щоб PostgreSQL міг вибирати оптимальні плани виконання запитів.

**Проста аналогія:** Уявіть, що ви збираєтеся в поїздку. Без статистики (які дороги, затори, відстані) ви будете вибирати маршрут навмання. ANALYZE — це як GPS, який знає стан усіх доріг і допомагає вибрати найкращий шлях.
...
**Приклад:**
```sql
-- Зібрати статистику за всією базою
ANALYZE;

-- За конкретною таблицею
ANALYZE users;

-- За конкретними колонками
...
**Важливо:** `ANALYZE` тільки збирає статистику. `VACUUM ANALYZE` робить і те, й інше: очищає dead tuples ТА збирає статистику.

**Навіщо:**
- Без статистики планувальник "вгадує" і може вибрати повільний план
...
```
Обсяг вибірки = 300 × statistics_target
...
| **most_common_vals** | Топ частих значень (MCV — Most Common Values) |
| **most_common_freqs** | Частота кожного з топ значень |
| **correlation** | Наскільки фізичний порядок збігається з логічним (-1 to 1) |
...
-- most_common_vals = ['Kyiv', 'Lviv', 'Odesa', ...]
-- most_common_freqs = [0.3, 0.2, 0.05, ...]
-- → Kyiv = 30% таблиці → Seq Scan (занадто багато для індексу)
...
-- Якщо n_mod_since_analyze великий → час для ANALYZE
```
...
1. Випадково вибирає сторінки з таблиці
2. Сканує всі рядки на вибраних сторінках
3. Будує статистику за вибіркою

Вибірка сторінок дає більш рівномірне покриття, враховуючи локальність
...
Недолік:
  - Для дуже "рваного" розподілу може бути неточним
```
...
-- Через помилку в 6 разів планувальник може вибрати Nested Loop (очікуючи 5
...
→ Якщо таблиці корелюють → оцінка помилкова
...
2. **Inheritance**
   ```sql
   -- Для таблиць зі спадкуванням:
   ANALYZE parent_table;  -- Тільки батько
   ANALYZE;               -- Всю БД, включаючи нащадків
...
|---|---|
| Занижена в 10x | Index Scan замість Seq Scan |
| Завищена в 10x | Seq Scan замість Index Scan |
| Неправильний JOIN order | Nested Loop замість Hash Join |

### Production Experience

**Реальний сценарій #1: Масова вставка без ANALYZE**
- ETL: завантажили 50 млн рядків в orders
- Запити стали в 100 разів повільнішими
...
- Рішення: `ANALYZE orders;` → 2 секунди, все полагодилося

**Реальний сценарій #2: Correlated columns**
- Аналітика: WHERE country = 'UA' AND city = 'Kyiv'
- Планувальник помилявся в 50 разів (припускав незалежність)
...
5. **Не покладайтеся тільки на autovacuum** для гарячих таблиць
...
- **statistics_target** — баланс між точністю та часом
...
- Extended Statistics (PG 10+): `dependencies` (корельовані колонки), `ndistinct` (складена кардинальність), `mcv` (PG 13+, топ комбінацій)
...
**Пов'язані теми:**
- [Що таке кардинальність індексу](06.%20Що%20таке%20кардинальність%20індексу.md) → n_distinct, MCV, correlation
- [Що таке VACUUM у PostgreSQL](18.%20Що%20таке%20VACUUM%20у%20PostgreSQL.md) → Autovacuum + ANALYZE
