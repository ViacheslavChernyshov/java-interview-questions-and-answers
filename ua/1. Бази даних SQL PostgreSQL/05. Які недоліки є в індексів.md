...
**Проста аналогія:** Уявіть, що у вас є книга зі змістом. Коли ви додаєте новий розділ, вам потрібно не лише вписати його в книгу, а й оновити зміст. Якщо змістів кілька — оновлювати потрібно кожен.
...
- ⚠️ **Вимагають обслуговування** — потрібно стежити за їхнім станом

**Приклад:**
```sql
-- Таблиця з 5 індексами
-- При вставці 1 рядка:
-- 1. Записуємо рядок у таблицю
...
**Правило:** Створюйте індекси тільки тоді, коли вони дійсно потрібні.

---

## 🟡 Middle Level

### Write Amplification (Посилення запису)
...
= 1 запис у таблицю 
+ N записів в індекси
+ Запис у WAL (Write Ahead Log)
```

**Приклад впливу на продуктивність:**

| Кількість індексів | INSERT latency | WAL обсяг |
|---|---|---|
| 1 | 1 ms | 1x |
| 5 | 5–10 ms | 5x |
| 10 | 20–50 ms | 10x |
...
**HOT (Heap Only Tuple)** — оптимізація PostgreSQL, яка дозволяє оновлювати рядок без оновлення індексів.

**Heap (купа)** — основне сховище таблиці, де лежать самі рядки даних. Індекс містить лише ключі + посилання (TID) на Heap. При HOT нова версія рядка записується на ту саму сторінку Heap із тим самим TID — індекси залишаються валідними, їх не потрібно чіпати.
...
-- ✅ Цей UPDATE може використовувати HOT (note не індексовано)
UPDATE orders SET note = 'New note' WHERE id = 123;
```

**Умова для HOT:**
- Змінені колонки **не індексовані**
...
- Таблиця < 10 000 рядків (Seq Scan швидший)
- Колонка з низькою кардинальністю (наприклад, стать: M/F)
- Поле, яке рідко використовується у WHERE/JOIN
...
### Write Amplification та WAL-логи
...
├── Можливо Page Splits: +10-20 записів
└── I/O операцій: 15-30
```

**Вплив на Replica Lag:**
- Кожен індекс генерує WAL-записи
- Репліка повинна застосувати ВСІ записи
- При 10+ індексах: WAL volume збільшується в 10–15 разів
- Репліка не встигає → Replica Lag зростає
- **Критично для систем із Read Replicas**
...
-- UPDATE Row2 без HOT:
-- 1. Створюємо нову версію Row2 на іншій сторінці
-- 2. Оновлюємо ВСІ індекси (нова TID)
-- 3. Стару версію позначаємо dead

-- UPDATE Row2 з HOT (є місце на сторінці):
-- 1. Створюємо нову версію Row2 поруч
-- 2. Оновлюємо тільки Heap (індекси не чіпаємо!)
-- 3. Стару версію позначаємо dead
-- 4. Індекси все ще валідні (TID не змінився)
```
...
-- hot_ratio > 70% → чудово (типове значення для здорового OLTP-навантаження)
-- hot_ratio < 20% → проблема: більшість оновлень змушені оновлювати індекси,
--   що вказує на надлишкове індексування або занадто малий fillfactor
```
...
**geqo_threshold:** При > 12 таблиць у запиті вмикається генетичний алгоритм (GEQO), який може обрати неоптимальний план.
...
- Симптом: Пошук за email > 500ms, Cache Hit rate 60%
...
6. **Стежте за Replica Lag** після додавання індексів
...
- **Planning Time** зростає з кількістю індексів
...
**Обов'язково знати:**
- Write Amplification: 1 INSERT = 1 запис у таблицю + N записів в індекси + WAL
- HOT (Heap Only Tuple): оптимізація UPDATE без оновлення індексів (якщо колонка не індексована)
- HOT ratio > 70% → чудово, < 20% → занадто багато індексів
...
- «Як перевірити Bloat індексу?» → `pgstattuple`, avg_leaf_density < 50% → REINDEX
- «Що таке HOT і чому це важливо?» → UPDATE без оновлення індексів → швидше
- «Чому Cache Hit Rate падає зі зростанням індексів?» → Індекси витісняють дані з RAM

**Червоні прапорці (НЕ говорити):**
- ❌ «Чим більше індексів, тим краще» (Write Amplification!)
- ❌ «VACUUM стискає файл індексу» (ні, REINDEX)
- ❌ «HOT — це якась магія» (UPDATE без оновлення індексів!)
- ❌ «Planning Time не важливий» (100+ індексів → > 100ms планування)

**Пов'язані теми:**
- [Для чого потрібні індекси](01.%20Для%20чого%20потрібні%20індекси.md) → коли створювати
- [Як працює B-tree індекс](02.%20Як%20працює%20B-tree%20індекс.md) → Bloat, Page Splits
- [Що таке складений індекс](03.%20Що%20таке%20складений%20індекс.md) → консолідація індексів
- [Як оптимізувати повільні запити](21.%20Як%20оптимізувати%20повільні%20запити.md) → баланс індексів
