# У чому різниця між INNER JOIN та LEFT JOIN?

---

## 🟢 Junior Level

**Головна різниця:** INNER JOIN повертає тільки рядки зі збігами в обох таблицях, а LEFT JOIN повертає **всі** рядки з лівої таблиці, навіть якщо збігів немає.

**Простий приклад:**
```sql
-- Таблиця users: Іваненко, Петренко, Сидоренко
-- Таблиця orders: Іваненко (замовлення), Петренко (замовлення)

-- INNER JOIN: тільки користувачі із замовленнями
SELECT u.name, o.amount
FROM users u
INNER JOIN orders o ON u.id = o.user_id;
-- Результат: Іваненко, Петренко

-- LEFT JOIN: усі користувачі, навіть без замовлень
SELECT u.name, o.amount
FROM users u
LEFT JOIN orders o ON u.id = o.user_id;
-- Результат: Іваненко (100), Петренко (200), Сидоренко (NULL)
```

**Коли використовувати:**
- **INNER JOIN** — коли потрібні тільки пов'язані дані
...
**INNER JOIN** — асоціативний і комутативний:
```sql
...
- **Hash Join** — будує хеш-таблицю за меншою таблицею й шукає збіги (швидко для великих таблиць без індексу)
- **Merge Join** — обидві таблиці сортуються та зливаються як два відсортовані масиви (швидко, якщо дані вже відсортовані)

### Фізичні стратегії

| Метрика | INNER JOIN | LEFT JOIN |
|---|---|---|
| **Hash Join** | Хеш будується за **меншою** таблицею | Хеш, як правило, за **правою** таблицею |
...
2. **Дублювання рядків**
   ...
   -- ✅ Групуйте або використовуйте EXISTS
...
2. **Коли ORM генерує LEFT JOIN «про всяк випадок»** — перевірте, чи реально потрібні дані з правої таблиці; якщо ні — Outer Join Removal видалить JOIN, але краще не генерувати його
...
Можливих порядків: тільки 1 (A → B → C → D)
Планувальник може тільки обирати фізичні стратегії.
```

**Вплив на продуктивність:**
- На складних схемах (10+ таблиць) ланцюжки LEFT JOIN можуть призвести до катастрофічно поганого плану
- **Рішення:** Використовуйте INNER JOIN де можливо, LEFT JOIN тільки де семантично необхідний
...
**Умови видалення:**
1. Не вибираєте жодної колонки з правої таблиці
2. Ключ у правій таблиці UNIQUE або PRIMARY KEY
...
**Навіщо це відбувається:**
- Часто ORM або універсальні Views генерують LEFT JOIN "про всяк випадок"
...
-- Значить, LEFT JOIN семантично еквівалентний INNER JOIN
```
...
LEFT JOIN orders o ON u.id = o.user_id
WHERE o.amount > 100;
-- У виведенні побачите: "Join Type: Inner"
```
...
-- Для кожного користувача → 3 замовлення з індексу
-- Чудово для users < 10,000
...
- Звіт: 15 таблиць, 12 із них через LEFT JOIN "про всяк випадок"
...
- Реальність: constraint був DEFERRABLE → оптимізатор не впевнений у UNIQUE
...
SELECT u.name FROM users u
LEFT JOIN orders o ON u.id = o.user_id;
-- Шукайте "Join Type: Inner" або відсутність orders у плані
...
-- 4. Статистика за JOIN у pg_stat_statements
...
2. **LEFT JOIN тільки де потрібно** зберегти всі записи з лівої таблиці
3. **Фільтри на правій таблиці** ставте в `ON`, а не у `WHERE`
...
7. **COALESCE** для агрегацій із LEFT JOIN
...
- Фільтри на правій таблиці → **в ON**, не у WHERE
...
**Обов'язково знати:**
- INNER JOIN: асоціативний і комутативний → оптимізатор може переставляти таблиці
...
- Hash Join: для LEFT JOIN хеш ЗАВЖДИ за правою таблицею (для INNER — гнучко)
- Nested Loop: для LEFT JOIN ліва таблиця ЗАВЖДИ driving
- Фільтри на правій таблиці → в ON (збереже NULL), не у WHERE (відфільтрує NULL)
...
- «Що буде, якщо фільтр у WHERE замість ON для LEFT JOIN?» → Відфільтрує NULL-рядки
...
- ❌ «Фільтр на правій таблиці можна у WHERE» (перетворить LEFT JOIN на INNER!)
- ❌ «SUM з LEFT JOIN поверне 0» (поверне NULL! Потрібен COALESCE)

**Пов'язані теми:**
- [Які типи JOIN існують](07.%20Які%20типи%20JOIN%20існують.md) → фізичні стратегії виконання
- [Що краще JOIN чи підзапит](09.%20Що%20краще%20JOIN%20чи%20підзапит.md) → коли EXISTS кращий за JOIN
- [Що таке корельований підзапит](10.%20Що%20таке%20корельований%20підзапит.md) → EXISTS як альтернатива
