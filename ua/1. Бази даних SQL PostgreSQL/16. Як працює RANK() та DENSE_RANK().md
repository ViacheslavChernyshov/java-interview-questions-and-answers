...
**RANK()** та **DENSE_RANK()** — віконні функції, які присвоюють ранг (місце) рядкам на основі їхніх значень, **однакові значення отримують однаковий ранг**.

**Проста аналогія:** Спортивні змагання:
- **RANK()** — олімпійська система: два золота → наступний бронзовий (пропуск місця)
- **DENSE_RANK()** — система грейдів: два золота → наступний срібний (без пропусків)
...
|---|---|---|---|
| **ROW_NUMBER()** | Різні номери | Немає | ✅ Завжди |
| **RANK()** | Однаковий ранг | ✅ Так | ❌ Ні |
| **DENSE_RANK()** | Однаковий ранг | ❌ Немає | ❌ Ні |

### Коли що вибирати

**RANK() — коли важливі "чесні місця":**
```sql
-- Топ-10 гравців (поверне всіх із 10-м місцем)
SELECT * FROM (
...
2. **LIMIT замість RANK для Top-N**
   ...
   -- Якщо 10-й і 11-й мають однаковий score → один відсічений випадково
...
2. **Коли ties не важливі** — простий LIMIT достатній
3. **При сотнях мільйонів рядків і важлива швидкість** — ROW_NUMBER швидший
...
- На 100 млн рядків: ROW_NUMBER у 2 рази швидший
...
**Сценарій:** Конкурс із призовими місцями
```sql
-- ❌ LIMIT несправедливий при однакових балах
SELECT * FROM contest ORDER BY score DESC LIMIT 10;
-- Якщо 10-й і 11-й набрали однаково → один не отримає приз

-- ✅ RANK гарантує чесність
SELECT * FROM (
    SELECT name, score,
           RANK() OVER(ORDER BY score DESC) as rank
    FROM contest
) t WHERE rank <= 10;
-- Поверне ВСІХ із рангом <= 10, навіть якщо їх 15
```
...
2. **PARTITION BY з RANK**
   ```sql
   -- Ранг усередині кожної групи
   SELECT department, name, salary,
          RANK() OVER(PARTITION BY department ORDER BY salary DESC) as dept_rank
   FROM employees;
   
   -- У кожному відділі свій ранг 1, 2, 3...
   ```

3. **RANK з декількома ORDER BY**
   ```sql
...
**Пам'ять для Sort:**
```sql
-- RANK/DENSE_RANK вимагають сортування
-- Якщо Sort не вміщується у work_mem → spill на диск
EXPLAIN (ANALYZE) SELECT ... RANK() OVER(ORDER BY ...);
-- "Disk: X kB" → збільшіть work_mem
```
...
- Запит із NTILE(4): 2 секунди
- Використання: таргетовані кампанії за квартилями
...
-- "Disk: X kB" → spill! Збільшіть work_mem

-- 3. Пошук запитів із ранжуванням
...
1. **RANK()** для чесних Top-N (усі рівні потрапляють)
...
8. **LIMIT vs RANK:** RANK для чесної вибірки, LIMIT для фіксованої кількості
...
**Обов'язково знати:**
- RANK(): однакові значення = однаковий ранг, наступний ранг із пропуском (1, 1, 3)
...
- NULLS LAST/FIRST: NULL за замовчуванням FIRST у ASC, LAST у DESC
- Індекс за `(PARTITION BY, ORDER BY)` → WindowAgg без Sort
...
- «Чому NULLS LAST потрібен?» → Без нього NULL отримають rank = 1 при DESC
...
- ❌ «RANK та ROW_NUMBER — одне й те саме» (RANK з пропусками, ROW_NUMBER унікальна)
...
**Пов'язані теми:**
- [Як працює ROW_NUMBER()](15.%20Як%20працює%20ROW_NUMBER().md) → відмінності від RANK
- [Що таке віконні функції](14.%20Що%20таке%20віконні%20функції.md) → загальний огляд
- [Як працює GROUP BY](12.%20Як%20працює%20GROUP%20BY.md) → NTILE як альтернатива групуванню
