...
**ROW_NUMBER()** — віконна функція, яка призначає унікальний порядковий номер кожному рядку.
...
```sql
ROW_NUMBER() OVER(
    PARTITION BY column1  -- Поділ на групи (опціонально)
...
**2. Дедуплікація:**
```sql
-- Залишити тільки останню версію запису
WITH ranked AS (
...
-- ⚠️ Незважаючи на LIMIT 10, база прочитає всіх користувачів!
```
...
1. **Для агрегацій** (COUNT, SUM, AVG) — використовуйте віконні агрегатні функції
...
-- → Тримає в пам'яті тільки топ-10 елементів (ніби ви відібрали
--   топ-10 кандидатів, не сортуючи всі 1000 резюме)
-- → Витісняє зайві в міру обробки
...
-- → Сортуємо тільки за created_at всередині кожної групи
-- → Набагато дешевше за повне сортування!
...
WHERE id > 100000  -- Останнє id з попередньої сторінки
ORDER BY id
LIMIT 20;
...
-- Для сторінки 50,000: ROW_NUMBER обробить 100,000 рядків, Keyset — тільки 20.
```
...
-- "external merge Disk: X kB" → збільшіть work_mem
...
**Паралелізм:**
```sql
-- WindowAgg НЕ паралелиться в PG < 14
...
- Запит із ROW_NUMBER + DELETE: 3 хвилини
- Рішення: 
...
- Результат: 50ms (прискорення в 160 разів)
...
-- 2. Перевірка Sort пам'яті
EXPLAIN (ANALYZE)
SELECT ... ROW_NUMBER() OVER(ORDER BY ...);
-- "Disk: X kB" → spill! Збільшіть work_mem

-- 3. Пошук запитів із ROW_NUMBER
...
4. **ROW_NUMBER** тільки для звітів/пагінації за номером сторінки
...
- **Фільтрація** тільки в підзапиті/CTE
...
- PARTITION BY — поділ на групи, ORDER BY — сортування всередині (обов'язковий!)
...
- «Коли COUNT(*) OVER() проблема?» → Незважаючи на LIMIT, читає ВСІ рядки

**Червоні прапорці (НЕ говорити):**
- ❌ «ROW_NUMBER можна фільтрувати у WHERE» (ні, після WHERE рахуються)
- ❌ «Keyset Pagination — це те саме, що ROW_NUMBER» (Keyset = Index Scan, ROW_NUMBER = Sort)
- ❌ `COUNT(*) OVER() OK з LIMIT` (читає ВСІ рядки!)

**Пов'язані теми:**
- [Як працює RANK() та DENSE_RANK()](16.%20Як%20працює%20RANK()%20та%20DENSE_RANK().md) → відмінності від ROW_NUMBER
- [Що таке віконні функції](14.%20Що%20таке%20віконні%20функції.md) → загальний огляд
- [Що таке explain plan](20.%20Що%20таке%20explain%20plan.md) → перевірка плану WindowAgg → Sort/Index Scan
