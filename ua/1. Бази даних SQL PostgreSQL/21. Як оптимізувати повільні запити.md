...
```sql
-- 1. Знайти повільний запит
-- (логи застосунку, pg_stat_statements)
...
-- Sort → Disk Spill → збільшіть work_mem
...
SELECT * FROM users WHERE phone = 380991234567;  -- phone = text

-- ✅ Правильний тип
SELECT * FROM users WHERE phone = '380991234567';
```
...
1. **Занадто багато індексів**
   ...
   - Видаляйте невикористовувані: `idx_scan = 0`
...
```sql
-- Проблема: запит швидкий у psql, повільний у застосунку
...
-- Рішення 2: відключити Prepared Statements
...
```sql
-- Перевірка неявних приведень типів
EXPLAIN (ANALYZE) SELECT * FROM users WHERE phone = 380991234567;
-- Filter: (phone = '380991234567'::text)  ← cast!
-- → Index не використовується

-- Рішення: типи, що збігаються
-- 1. Змінити тип колонки
ALTER TABLE users ALTER COLUMN phone TYPE bigint;

-- 2. Або передавати правильний тип
SELECT * FROM users WHERE phone = '380991234567';  -- text
```
...
-- WHERE created_at > '2024-01-01' → сканує тільки orders_y2024
```
...
pgBouncer у режимі transaction:
  - 1000 з'єднань застосунку → 50 з'єднань до БД
...
ОС + PostgreSQL:
  ...
  - Менше TLB misses
    ...
    При 4KB для 8GB RAM → 2 млн записів TLB (не вміщується).
    При 2MB → 4000 записів (все в TLB))
...
- **Materialized Views:** НЕ використовуйте для даних, які повинні бути актуальними в реальному часі (затримка між REFRESH)
- **Partitioning:** НЕ використовуйте для таблиць < 10 млн рядків — overhead управління партиціями перевищує вигоду
...
- Результат: p95 < 200ms (прискорення в 10 разів)
...
8. **pgBouncer** рекомендований при > 50 підключень. Для малих проєктів (< 10 користувачів) можна обійтися без нього.
9. **Partitioning** для таблиць > 100 млн рядків
10. **Cache Hit Ratio > 99%** — мета

### Резюме для Senior

- Оптимізація — **не тільки індекси**, це системний процес
- **Plan Cache** (Generic vs Custom) — часта причина prod/dev розходжень
- **Implicit Casts** блокують індекси → типи, що збігаються
...
- **Write Amplification** (посилення запису): кожен INSERT/UPDATE записує не тільки в таблицю, але й у ВСІ індекси. 10 індексів = запис × 10. Фізично записується в 10x більше даних, ніж змінив користувач.
...
- Часті проблеми: немає індексу, функція у WHERE, неявний cast, spill на диск, N+1
- Generic vs Custom Plans: Prepared Statements → plan_cache_mode = force_custom_plan
- Implicit Casts: `phone = 38099...` (int) vs `phone = '38099...'` (text) → cast блокує індекс
...
- «Чому запит швидкий у консолі, повільний у застосунку?» → Generic Plan
- «Що робити при Disk Spill?» → SET work_mem = '256MB' для сесії
- «Коли денормалізація виправдана?» → Hot-path запити, JOIN 10+ таблиць

**Червоні прапорці (НЕ говорити):**
- ❌ «Оптимізація = тільки індекси» (системний процес: SQL, конфіг, архітектура, інфраструктура)
- ❌ «plan_cache_mode не важливий» (Generic Plan — часта причина розходжень)
- ❌ «Materialized Views для OLTP» (тільки для аналітики!)
- ❌ «pgBouncer не потрібен» (1000 з'єднань = 30% CPU на перемиканні)

**Пов'язані теми:**
- [Для чого потрібні індекси](01.%20Для%20чого%20потрібні%20індекси.md) → перше рішення для оптимізації
- [Що таке explain plan](20.%20Що%20таке%20explain%20plan.md) → EXPLAIN (ANALYZE, BUFFERS)
- [Навіщо потрібен ANALYZE](19.%20Навіщо%20потрібен%20ANALYZE.md) → статистика для планувальника
- [Як працює MVCC у PostgreSQL](17.%20Як%20працює%20MVCC%20у%20PostgreSQL.md) → Bloat і довгі транзакції
