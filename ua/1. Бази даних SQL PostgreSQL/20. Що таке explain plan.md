...
- Перед створенням індексу — чи допоможе він
...
```sql
-- EXPLAIN: тільки план, НЕ виконує запит
EXPLAIN SELECT * FROM users WHERE email = 'test@example.com';
...
-- ROWS     → показати очікувану к-ть рядків
...
> Bitmap Index Scan: збирає ВСІ matching TID з індексу в бітову карту, потім читає рядки. Вигідніше за Index Scan для 100–10,000 рядків із різних місць — менше випадкових I/O.
...
2. **Оцінка не збігається з реальністю**
   ...
   -- Планувальник помилився в 1000 разів!
...
- Планувальник може вибрати план з величезним Total Cost, але низьким Startup Cost
...
**Інтерпретація:**
- `shared hit` >> `shared read` → чудово (все в кеші)
...
для id=100 — Seq Scan (500,000 рядків). Seq Scan «безпечніший» у середньому,
...
-- 2. У JDBC/Hibernate:
-- Перевірте: prepareThreshold=0 (відключити Prepared Statements)
```
...
```sql
-- В EXPLAIN ANALYZE може з'явитися блок JIT
-- JIT:
--   Functions: 5
...
-- Workers Launched: скільки реально запустилося
...
--   → Sort тільки за created_at всередині кожного user_id
...
1. **EXPLAIN ANALYZE виконує запит!**
   ```sql
   -- ⚠️ DML запит буде виконаний!
   EXPLAIN ANALYZE DELETE FROM users WHERE status = 'INACTIVE';
   -- → Усі неактивні користувачі ВИДАЛЕНІ!
...
2. **Перший запуск EXPLAIN ANALYZE може брехати**
   ```
   Дані можуть бути не в кеші → перший запуск повільніший.
   Запустіть 2-3 рази й дивіться на другий/третій результат.
...
3. **WAL генерація (PG 13+)**
   ...
   --   першій зміні сторінки після checkpoint. Велика кількість FPI =
   --   високе навантаження на WAL та реплікацію)
   
   -- Критично для оцінки навантаження на реплікацію
   ```

> На PG 12 і нижче прапор WAL в EXPLAIN недоступний. Оцінюйте навантаження на реплікацію через `pg_stat_wal` або логи.

4. **Timing overhead**
   ```sql
   -- EXPLAIN ANALYZE додає overhead на заміри часу
   -- Для мікро-оптимізацій відключіть TIMING
...
- Застосунок: JDBC Prepared Statement для пошуку за email
- У консолі: 5ms (Custom Plan, Index Scan)
- У застосунку: 500ms (Generic Plan, Seq Scan)
...
- Результат: 0.5ms замість 8.5ms (прискорення в 17 разів)
...
-- 3. Пошук запитів із Generic Plan проблемами
-- (у логах застосунку: повільно, у консолі: швидко)

-- 4. Перевірка JIT
EXPLAIN (ANALYZE) SELECT ...;
-- Шукайте блок "JIT" та Timing
...
2. **Порівнюйте Estimated vs Actual rows** → розходження = проблема статистики
3. **Перевіряйте Cache Hit Rate** через Buffers
4. **Generic vs Custom** — тестуйте Prepared Statements
5. **JIT** відключайте для OLTP, вмикайте для OLAP
6. **DML + EXPLAIN ANALYZE** → тільки в транзакції з ROLLBACK
...
- **Generic vs Custom Plans** — часта причина розходжень prod/dev
- **plan_cache_mode** керує поведінкою Prepared Statements
- **JIT** прискорює OLAP, сповільнює OLTP → відключайте де не потрібно
- **WAL (PG 13+)** показує навантаження на реплікацію
- **Estimated ≠ Actual rows** → `ANALYZE` або `SET STATISTICS`
- **DML + EXPLAIN ANALYZE** → завжди в `BEGIN...ROLLBACK`
...
- DML + EXPLAIN ANALYZE → ВИКОНУЄ запит! Тільки в BEGIN...ROLLBACK

**Часті уточнюючі питання:**
- «Чому в застосунку повільніше, ніж у консолі?» → Generic Plan у Prepared Statements
- «Як перевірити Cache Hit Rate?» → BUFFERS: hit / (hit + read)
- «Чому JIT сповільнює?» → Компіляція 8ms > виконання 1ms → відключіть для OLTP
- «Що означає Estimated rows ≠ Actual?» → Застаріла статистика → ANALYZE

**Червоні прапорці (НЕ говорити):**
- ❌ «EXPLAIN ANALYZE безпечний для DML» (ВИКОНУЄ запит!)
- ❌ «Generic Plan завжди хороший» (для skewed даних — катастрофа)
- ❌ «JIT завжди прискорює» (сповільнює OLTP!)
- ❌ «Cost — це мілісекунди» (ні, умовні одиниці!)

**Пов'язані теми:**
- [Як оптимізувати повільні запити](21.%20Як%20оптимізувати%20повільні%20запити.md) → практичне застосування EXPLAIN
- [Навіщо потрібен ANALYZE](19.%20Навіщо%20потрібен%20ANALYZE.md) → Estimated ≠ Actual → ANALYZE
- [Що таке корельований підзапит](10.%20Що%20таке%20корельований%20підзапит.md) → пошук SubPlan у плані
