...
- Список учнів з їхніми ID
- Список оцінок з ID учня

JOIN дозволяє з'єднати ці списки й побачити: "Іваненко (ID=1) — оцінка 5, Петренко (ID=2) — оцінка 4".
...
| **CROSS JOIN** | Усі з усіма (декартів добуток) | "Згенерувати всі комбінації" |
...
2. Скануємо більшу таблицю, шукаємо збіги в хеші
```
- **Коли використовується:** Великі таблиці, немає відповідного індексу
...
-- Semi-Join ефективніший за звичайний JOIN: щойно знайдено ПЕРШИЙ збіг
...
1. **Використання JOIN замість EXISTS**
   ...
   -- ✅ EXISTS ефективніший (зупиняється після першого збігу)
   SELECT * FROM users u
   WHERE EXISTS (
       SELECT 1 FROM orders o WHERE o.user_id = u.id
   );
...
```sql
-- Якщо хеш-таблиця не вміщується у work_mem → скидання на диск
EXPLAIN ANALYZE SELECT * FROM orders o
JOIN users u ON o.user_id = u.id;

-- Якщо бачите: "Batches: 4" → хеш не вмістився в пам'ять
...
| LEFT JOIN | Потрібно зберегти все з лівої | Залежить від даних |
...
**Складність:**
- Без індексу: O(N × M) — катастрофа на великих даних
- З індексом: O(N × log M) — чудово
...
- Немає відповідного індексу
- Рівність в умові JOIN (`=`)

**Batches (коли не вміщується у work_mem):**
```
...
-- 1. Не вибираємо колонки з правої таблиці
-- 2. Ключ у правій таблиці UNIQUE/PRIMARY KEY
...
3. **Lateral Join та Nested Loop**
   ...
   -- Чудово для малих зовнішніх вибірок
...
-- ⚠️ ТІЛЬКИ для налагодження! Не у production!
...
- Аналітична платформа: JOIN таблиць з 50 млн і 100 млн рядків
...
4. **work_mem** збільшуйте для конкретних запитів із Hash Join
5. **Стежте за Batches** в EXPLAIN — якщо > 1, збільшуйте work_mem
...
- **3 фізичні стратегії:** Nested Loop (малі + індекс), Hash (великі без індексу), Merge (відсортовані)
- **GEQO** при > 12 таблиць → може обрати поганий план
- **Batches > 1** у Hash Join → збільшуйте `work_mem`
...
- Nested Loop: O(N × log M) з індексом, чудово для малих + індекс
- Hash Join: O(N + M), вимагає work_mem, Batches > 1 → spill на диск
- Merge Join: вимагає відсортованих даних, O(N + M) після сортування
- GEQO при > 12 таблиць → генетичний алгоритм → може обрати поганий план
- LATERAL ≈ Nested Loop → тільки для < 10,000 зовнішніх рядків
- Semi-Join (EXISTS) ефективніший за DISTINCT JOIN (немає дублікатів)

**Часті уточнюючі питання:**
- «Чому Hash Join повільний?» → Batches > 1 → disk spill → збільшіть work_mem
...
- ❌ «LATERAL JOIN вирішить усе» (тільки для малих зовнішніх вибірок!)
- ❌ «NOT IN безпечний» (NULL поверне порожній результат!)

**Пов'язані теми:**
- [У чому різниця між INNER JOIN та LEFT JOIN](08.%20У%20чому%20різниця%20між%20INNER%20JOIN%20та%20LEFT%20JOIN.md) → логічні vs фізичні JOIN
- [Що краще JOIN чи підзапит](09.%20Що%20краще%20JOIN%20чи%20підзапит.md) → Semi-Join, декореляція
- [Для чого потрібні індекси](01.%20Для%20чого%20потрібні%20індекси.md) → індекси на полях JOIN
- [Що таке корельований підзапит](10.%20Що%20таке%20корельований%20підзапит.md) → EXISTS vs JOIN
