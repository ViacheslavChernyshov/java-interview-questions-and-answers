...
**Навіщо потрібен:**
- У PostgreSQL `UPDATE` не перезаписує рядок, а створює новий. Старий залишається "мертвим".
- `DELETE` не видаляє рядок фізично, а позначає як видалений.
- Без VACUUM таблиця буде зростати нескінченно.
...
**Важливо:**
- Звичайний VACUUM **в основному не повертає місце** операційній системі із середини файлу (звільняє блоки всередині файлу для нових рядків)
- У PostgreSQL 12+ звичайний VACUUM може повернути місце ОС із **кінця файлу** (`vacuum_truncate`), якщо хвостові сторінки виявилися повністю порожніми
- `VACUUM FULL` повністю перебудовує файл і повертає місце ОС, але **блокує таблицю** (AccessExclusiveLock)
...
-- Рядок позначений як видалений (dead tuple)
...
-- Зверніть увагу: для великих таблиць scale_factor 20% → autovacuum запускається
...
```sql
-- Для великої таблиці: частіше та агресивніше
...
ALTER TABLE settings SET (
    autovacuum_enabled = false  -- Відключити (якщо рідко змінюється)
);
```
...
3. **VACUUM FULL заблокував таблицю**
   ```sql
   -- ❌ У production!
   VACUUM FULL orders;  -- Усі INSERT/UPDATE/DELETE чекають!
...
> Це НЕ окремий процес. PostgreSQL видаляє старі версії з HOT-ланцюжків під час звичайних операцій, якщо всі версії на сторінці стали невидимими. «Миттєве» очищення без autovacuum.
...
-- Особливо для таблиць із частими UPDATE
```
...
-- Для таблиць із безліччю індексів → прискорення в 3–5 разів
```

> На PG 12 і нижче VACUUM виконується в одному потоці. Для прискорення: налаштуйте cost-based VACUUM (збільшіть cost_limit, зменшіть delay).
...
-- → Агресивний VACUUM:
--    - Неможливо скасувати (CANCEL не працює)
--    - Буде працювати, поки не "заморозить" усе
...
-- Екстрене очищення тільки Heap
-- → Швидше, але індекси не оновлюються
-- → Використовувати ТІЛЬКИ якщо індекси в порядку
```
...
-- ❌ Перестворює таблицю та ВСІ індекси
-- ✅ Повертає місце ОС
...
1. **hot_standby_feedback та Bloat**
   ```sql
   -- На репліці:
   hot_standby_feedback = on
   
   -- Репліка просить майстер не видаляти рядки
   -- → Майстер накопичує dead tuples
   -- → Bloat на майстрі!
...
- Bloat: таблиця 100 ГБ (повинна бути 10 ГБ)
- Рішення:
  ```sql
  ALTER TABLE orders SET (
...
- База: autovacuum відключений 2 роки "для продуктивності"
...
- Урок: НЕ відключайте autovacuum ГЛОБАЛЬНО. Для окремих таблиць при bulk load — можна тимчасово, але одразу ввімкніть назад.
...
1. **НЕ відключайте autovacuum ГЛОБАЛЬНО** — налаштовуйте. Для окремих
   рідко оновлюваних таблиць (довідники) можна локально відключити,
...
7. **Parallel VACUUM (PG 13+)** для таблиць із безліччю індексів
...
- Index-Only VACUUM (PG 14+): `VACUUM (INDEX_CLEANUP false)` — тільки Heap

**Часті уточнюючі питання:**
- «Чому autovacuum не справляється?» → scale_factor 20% занадто багато для гарячих таблиць
...
- ❌ «VACUUM повертає місце ОС» (ні, тільки позначає вільним)
- ❌ «autovacuum можна відключити» (Wraparound → READ ONLY!)
- ❌ «VACUUM FULL безпечний у production» (блокує ВСІ операції!)
- ❌ «hot_standby_feedback без наслідків» (Bloat на майстрі!)

**Пов'язані теми:**
- [Як працює MVCC у PostgreSQL](17.%20Як%20працює%20MVCC%20у%20PostgreSQL.md) → dead tuples, Hint Bits, XID Wraparound
- [Навіщо потрібен ANALYZE](19.%20Навіщо%20потрібен%20ANALYZE.md) → Autovacuum запускає ANALYZE
- [Як оптимізувати повільні запити](21.%20Як%20оптимізувати%20повільні%20запити.md) → Bloat впливає на продуктивність
