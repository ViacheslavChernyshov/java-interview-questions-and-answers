...
**Навіщо це розуміти:** такі підзапити виглядають елегантно та інтуїтивно зрозуміло, але на великих таблицях можуть паралізувати базу даних — підзапит виконується для кожного рядка зовнішнього запиту, що дає складність O(N × M).

**Проста аналогія:** Уявіть, що ви перевіряєте кожен документ у папці. Для кожного документа ви відкриваєте окрему папку й шукаєте там пов'язані файли. Це як цикл у коді: для кожного елемента → виконати пошук.
...
Класична модель: **O(N × M)** — підзапит виконується N разів (за кількістю рядків зовнішнього запиту).

```sql
-- Корельований підзапит у SELECT
SELECT u.name,
...
-- Оптимізатор ігнорує SELECT list усередині EXISTS
...
2. **Корельований UPDATE без індексу**
   ```sql
   -- ❌ Якщо orders.user_id не індексований → Seq Scan у циклі!
...
3. **Ігнорування Memoize (PG 14+)**
   ```sql
   -- На PG 14+ з Memoize може бути швидко навіть із LATERAL
   -- Перевіряйте EXPLAIN, не оптимізуйте передчасно
   ```
...
|---|---|---|
| Агрегація для кожного рядка | Підзапит у SELECT | **JOIN + GROUP BY** |
| Пошук одного значення | Підзапит із LIMIT 1 | **LATERAL JOIN** |
| Перевірка існування | Підзапит у WHERE | **EXISTS (Semi-Join)** |
...
-- Віконна функція робить один прохід по даних із сортуванням: O(N log N).
-- Корельований підзапит виконує окремий запит для кожного рядка:
-- O(N × M). На таблиці з 1 млн рядків це 1 млн окремих запитів
-- проти одного проходу — різниця в сотні й тисячі разів.
...
→ Може використовувати індекс за параметром
→ Може бути закешований (Memoize)
```
...
```sql
-- Початковий: корельований EXISTS
SELECT * FROM users u
WHERE EXISTS (
...
Hit rate = 950,000 / 1,000,000 = 95% → чудово!
...
- Працює тільки в межах одного вузла Nested Loop
...
```sql
-- Крок 1: Попередньо обчислюємо мінімум один раз
...
-- Віконна: O(N log N) — один прохід із сортуванням/хешуванням
```
...
- Запит: скалярний підзапит у SELECT (50,000 контактів)
...
-- 3. Пошук корельованих підзапитів у pg_stat_statements
...
1. **Уникайте скалярних підзапитів у SELECT** на великих таблицях
...
- **UPDATE FROM** / **DELETE USING** — завжди швидші за корельовані підзапити
...
- Memoize (PG 14+): кешує результати → Hit rate > 80% = чудово
- EXISTS = Semi-Join: зупиняється після першого збігу, немає дублів
- Скалярний підзапит у SELECT → JOIN + GROUP BY краще
...
- «Як знайти SubPlan у плані?» → Шукайте `SubPlan` в EXPLAIN (виконується для кожного рядка)
...
- ❌ «LATERAL JOIN завжди добрий» (Nested Loop → тільки для малих зовнішніх)

**Пов'язані теми:**
- [Що краще JOIN чи підзапит](09.%20Що%20краще%20JOIN%20чи%20підзапит.md) → Unnesting, Decorrelation
- [Які типи JOIN існують](07.%20Які%20типи%20JOIN%20існують.md) → Semi-Join, Hash Join
- [Що таке віконні функції](14.%20Що%20таке%20віконні%20функції.md) → альтернатива для агрегації на рядок
- [Що таке explain plan](20.%20Що%20таке%20explain%20plan.md) → пошук SubPlan у плані
