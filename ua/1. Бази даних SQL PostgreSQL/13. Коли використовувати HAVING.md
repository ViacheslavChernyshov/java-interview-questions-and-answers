...
**HAVING** використовується, коли потрібно відфільтрувати результати **після** групування за агрегатними функціями (`COUNT`, `SUM`, `AVG` тощо).
...
**Приклад:**
```sql
-- Знайти користувачів із більш ніж 5 замовленнями
...
Правильний поділ фільтрів — ознака хорошого SQL:
...
```sql
-- Проєкти з проблемами (більше 2 заблокованих завдань)
SELECT project_id
FROM tasks
GROUP BY project_id
HAVING COUNT(*) > 5
...
```sql
-- Перевірка умови за всією таблицею
...
1. **Для неагрегатних фільтрів** — `HAVING city = 'Kyiv'` групує ВСІ міста, потім фільтрує. Перенесіть у WHERE
...
2. Якщо не вміщується → spill на диск (temp_files — тимчасові файли на диску)
3. Продуктивність падає в 10–100 разів (операції з диском у мільйони разів повільніші за RAM)
...
-- 2. Матеріалізоване представлення для частих запитів
...
### FILTER в HAVING для декларативності
...
2. **HAVING з віконними функціями** — НЕ МОЖНА
   ```sql
...
-- HashAggregate (disk: Y kB)          → spill відбувся! Збільшіть work_mem
```

### Production Experience

**Реальний сценарій:**
- E-commerce: звіт з продажу (500 млн рядків)
...
- Результат: 10 секунд (прискорення в 90 разів)
- Чому: ORM генерував `HAVING status = 'COMPLETED'` — база спочатку групувала всі 500 млн рядків (включаючи скасовані, повернуті тощо), створюючи мільйони груп, і тільки потім фільтрувала. Перенесення у WHERE скоротило вхідні дані для GROUP BY у 10 разів + дозволило використати індекс за status.

### Monitoring

```sql
-- 1. Пошук запитів із HAVING у pg_stat_statements
...
3. **Уникайте аліасів** із SELECT у HAVING (непереносно)
...
- **Віконні функції** недоступні в HAVING → CTE/підзапит
- **FILTER** для декларативності складних умов
- **Переносність:** не використовуйте аліаси із SELECT у HAVING
...
- Filter Push-down (PG 12+): автоматичне перенесення з HAVING у WHERE, але НЕ завжди
...
- Аліаси із SELECT у HAVING: НЕ переносно (SQL Server/Oracle не підтримують)
...
- «Чи можна FILTER в HAVING?» → Так, елегантно для складних умов
- «Коли HAVING з підзапитом виправданий?» → Порівняння групи із середньою за всіма

**Червоні прапорці (НЕ говорити):**
- ❌ «HAVING скорочує дані для GROUP BY» (ні, фільтрує ВИХІД)
- ❌ «Можна покластися на Filter Push-down» (не завжди працює!)
- ❌ «Віконні функції можна в HAVING» (ПІСЛЯ HAVING рахуються!)

**Пов'язані теми:**
- [У чому різниця між WHERE та HAVING](11.%20У%20чому%20різниця%20між%20WHERE%20та%20HAVING.md) → порядок виконання
- [Як працює GROUP BY](12.%20Як%20працює%20GROUP%20BY.md) → huge number of groups
- [Що таке віконні функції](14.%20Що%20таке%20віконні%20функції.md) → чому не можна в HAVING
