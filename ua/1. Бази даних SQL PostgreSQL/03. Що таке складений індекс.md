...
**Складений (композитний) індекс** — це індекс, створений на декількох колонках одночасно.

**Навіщо:** один складений індекс може замінити кілька окремих, заощаджуючи місце на диску та прискорюючи запис. Крім того, він дозволяє оптимізувати запити, які фільтрують одразу за декількома полями — що зустрічається в більшості реальних додатків.
...
**Основна ідея:**
- Один індекс може прискорити пошук за декількома полями
...
-- ✅ Буде використано (за city)
SELECT * FROM users WHERE city = 'Kyiv';

-- ✅ Буде використано (за city та age)
SELECT * FROM users WHERE city = 'Kyiv' AND age = 25;

-- ❌ НЕ буде використано (тільки age)
...
- Для оптимізації сортування за декількома полями
...
1. Спочатку за першою колонкою
2. У межах однакових значень першої — за другою
3. І так далі
...
```sql
-- Запит: WHERE city = 'Kyiv' AND age > 25

-- ✅ Правильний порядок
CREATE INDEX idx_city_age ON users(city, age);

-- ❌ Неправильний порядок (age як діапазон зробить city марним)
CREATE INDEX idx_age_city ON users(age, city);
```

**2. Сортування (ORDER BY)**
```sql
-- Індекс покриває ORDER BY у тому самому порядку
...
SELECT * FROM orders ORDER BY created_at DESC; -- немає фільтра за user_id
```
...
1. **Неправильний порядок колонок**
   ```sql
   -- ❌ Якщо city використовується з '=', а age з '>', city має бути першим
...
3. **Занадто багато колонок**
   ...
   - На практиці: понад 5 колонок — червоний прапорець
...
| `WHERE A = 1 AND B = 2` | ⚠️ Bitmap або один із них | ✅ Повністю використовує |
...
-- Plan: Index Scan (за customer_id)
...
**Лексикографічне сортування:** Ключ складеного індексу — це фактично один великий рядок. У B-tree зберігається конкатенація значень колонок.

**Структура ключа в Leaf Node:**
```
┌─────────────────────────────────────────┐
│ City="Kyiv"   | Age=25 | TID=(123,456)  │
├─────────────────────────────────────────┤
│ City="Kyiv"   | Age=26 | TID=(123,457)  │
├─────────────────────────────────────────┤
│ City="Lviv"   | Age=20 | TID=(124,100) │
└─────────────────────────────────────────┘
```
...
**Приклад:**
```sql
-- Запит: WHERE status = 'ACTIVE' AND city = 'Kyiv' AND created_at > '2024-01-01'
-- ORDER BY created_at DESC
...
PostgreSQL **не має** вбудованого Skip Scan (на відміну від Oracle/MySQL 8.0). Але його можна емулювати:

```sql
-- Проблема: індекс (status, user_id), але шукаємо тільки за user_id
...
3. **Index-Only Scan та Visibility Map**
   - Навіть якщо всі колонки є в індексі, PG може звернутися до Heap
...
- Рішення:
  - Консолідували до 3 складених індексів
...
```sql
-- Перевірка використання складених індексів
SELECT 
    indexrelname,
    idx_scan,
    idx_tup_read,
...
2. **Консолідація:** Один індекс `(a,b,c)` замінює `(a)` і `(a,b)`
...
- Емулюйте **Skip Scan** через Recursive CTE для низькокардинальних перших колонок
- **Консолідуйте** індекси для економії RAM і прискорення запису
- **Visibility Map** впливає на Index-Only Scan — стежте за VACUUM
...
- Консолідація: `(a)`, `(a,b)`, `(a,b,c)` → один `(a,b,c)` заощаджує місце
...
- «Як замінити 3 окремі індекси одним?» → Складений `(a,b,c)` покриває всі префікси
- «Що таке Skip Scan і чому PG його не має?» → Пропуск першої колонки; у PG немає, емулюємо через CTE
- «Як ORDER BY впливає на порядок колонок?» → Індекс має збігатися з ORDER BY
...
**Пов'язані теми:**
- [Для чого потрібні індекси](01.%20Для%20чого%20потрібні%20індекси.md) → коли створювати індекси
- [Як працює B-tree індекс](02.%20Як%20працює%20B-tree%20індекс.md) → внутрішня структура
- [Що таке кардинальність індексу](06.%20Що%20таке%20кардинальність%20індекси.md) → порядок колонок за кардинальністю
- [Які недоліки є в індексів](05.%20Які%20недоліки%20є%20в%20індексів.md) → вартість запису
