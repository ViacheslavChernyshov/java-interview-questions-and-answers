...
Проблеми Singleton діляться на три категорії: **тестованість** (неможливо замокати), **архітектура** (приховані залежності), **інфраструктура** (проблеми з масштабуванням).
...
**Flaky Tests** — тести, які випадково проходять/падають без зміни коду.
...
@Test
void test1() {
    Counter counter = new Counter();  // Новий для кожного тесту
    counter.increment();
...
public class DataLoader {
    ...
    private List<Data> cache = new ArrayList<>();  // Зростає!
...
// Старий ClassLoader не збирається GC → Metaspace Leak
// → OutOfMemoryError: Metaspace
```

### 5. Не працює в кластері
...
| **Liskov Substitution** | Не можна підмінити спадкоємцем |
| **Interface Segregation** | Залежні від конкретної реалізації |
| **Dependency Inversion** | Залежні від конкретики, не від абстракції |
...
public class ConnectionPool {
    ...
    // Як закрити при зупинці застосунку?
...
public class MetricsCollector {
    ...
    public Map<String, Long> getMetrics() {
        return new HashMap<>(metrics);  // ...copy щоразу!
    }
}

// При 10,000 req/sec:
// ConcurrentHashMap знижує contention, але copy в getMetrics() створює 10,000 об'єктів/сек → GC pressure.
...
4. Усі класи webapp залишаються в пам'яті
5. Metaspace зростає → OutOfMemoryError
...
**Реальний сценарій #1: Singleton вбив масштабування**
...
1. **НЕ використовуйте ручні Singleton** у Spring-застосунках
2. **DI Container** керує lifecycle краще
3. **Уникайте мутабельного стану** в Singleton
...
- **SOLID violations:** Singleton порушує 5 із 6 принципів
- **Static deadlocks:** важко відловити, катастрофічно
...
**Обов'язково знати:**
- Singleton порушує SOLID: SRP (управління lifecycle), DIP (приховані залежності), OCP (неможливо розширити), LSP (не можна підмінити), ISP (залежність від конкретики)
...
- **Що таке static initialization deadlock?** — Два класи в статичних блоках чекають один одного → deadlock на етапі Class Loading
- **Чому Singleton тече в Tomcat?** — Static поле тримає ClassLoader → ClassLoader не GC → Metaspace зростає
...
- [Як реалізувати потокобезпечний Singleton](04.%20Як%20реалізувати%20потокобезпечний%20Singleton.md) — безпечні реалізації
- [Що таке double-checked locking](05.%20Що%20таке%20double-checked%20locking.md) — DCL оптимізація
- [Які категорії патернів існують](02.%20Які%20категорії%20патернів%20існують.md) — Creational патерни
- [Які антипатерни ви знаєте](16.%20Які%20антипатерни%20ви%20знаєте.md) — Singleton як антипатерн
