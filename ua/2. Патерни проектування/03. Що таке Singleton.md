...
**Singleton** — патерн, який гарантує, що у класу буде **тільки один екземпляр**.
...
**Проста аналогія:** У країні може бути тільки один президент. Якщо ви запитаєте "хто президент?", вам завжди назвуть одну й ту саму людину.
...
Singleton підтримує ліниву ініціалізацію, static class ініціалізується при завантаженні класу.
...
// ✅ Потокобезпечна
// ❌ Не лінива (створюється навіть якщо не потрібна)
```
...
private Object readResolve() {
    return getInstance();  // Повернути існуючий
}
```
...
2. **Стан у Singleton**
   ```java
   // ❌ Мутабельний Singleton = проблеми в багатопоточності
...
| **Розподілена система** | **НЕ ІСНУЄ** (потрібне розподілене блокування) |
...
JMM (Java Memory Model) — специфікація, яка визначає, як потоки бачать пам'ять один одного.
Bill Pugh Singleton — названий за ім'ям дослідника, який запропонував рішення через static inner class.
...
- Без volatile: інструкція reorder (виділення пам'яті → публікація → конструктор)
...
1. **Tight Coupling**
   ```java
   // Залежність не видна в сигнатурі
   public class OrderService {
       public void process() {
...
3. **Порушення SRP**
   ...
   // Клас відповідає за:
   // 1. Бізнес-логіку
   // 2. Управління своїм життєвим циклом
...
4. **Memory Leak**
   ...
   // У Tomcat при redeploy → старий ClassLoader не збирається GC
   // → Metaspace Leak
   ```
...
// JVM гарантує:
// 1. Лінивість (клас завантажиться тільки при першому виклику)
...
```java
// Singleton НЕ буде видалений GC, поки:
// 1. Живий ClassLoader
// 2. Є посилання на статичне поле

// У стандартній Java: ClassLoader живий до кінця процесу
...
**Реальний сценарій #1: Singleton вбив тестованість**
...
1. **НЕ пишіть Singleton вручну** у Spring-застосунках
2. **Використовуйте DI** (`@Component` = Singleton scope)
3. **Enum** — safest для бібліотек/SDK
4. **Bill Pugh** — для лінивої ініціалізації
5. **Уникайте мутабельного стану** в Singleton
6. **readResolve()** для захисту від серіалізації
7. **Перевірка в конструкторі** для захисту від рефлексії
...
- **Bill Pugh** = лінивість + потокобезпечність без synchronized
...
- Bill Pugh (Static Holder) — лінива ініціалізація без synchronized, через static inner class
- У Spring @Component = Singleton scope за замовчуванням, НЕ пишіть Singleton вручну
- Singleton не працює в кластері — кожна нода створює свій екземпляр
- Без volatile в DCL можливий reordering: потік отримає неініціалізований об'єкт
...
- **Коли Singleton припустимий?** — Логери, конфігурація, кеші — stateless або імутабельні об'єкти
- **Що таке Bill Pugh Singleton?** — Лінива ініціалізація через static inner class, JVM гарантує потокобезпечність

**Червоні прапорці (НЕ говорити):**
- ❌ «Я використовую Singleton для зберігання стану в Spring» — Spring керує цим через DI
- ❌ «Singleton працює в кластері» — кожна нода створює свій екземпляр
...
**Пов'язані теми:**
- [Як реалізувати потокобезпечний Singleton](04.%20Як%20реалізувати%20потокобезпечний%20Singleton.md) — способи реалізації
- [Що таке double-checked locking](05.%20Що%20таке%20double-checked%20locking.md) — оптимізація для Singleton
- [Які проблеми є у Singleton](06.%20Які%20проблеми%20є%20у%20Singleton.md) — антипатерни та проблеми
- [Які категорії патернів існують](02.%20Які%20категорії%20патернів%20існують.md) — Singleton як Creational патерн
- [Які антипатерни ви знаєте](16.%20Які%20антипатерни%20ви%20знаєте.md) — Singleton як джерело проблем
