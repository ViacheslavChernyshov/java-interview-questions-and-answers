...
Патерни діляться на **три основні категорії** залежно від того, що вони допомагають робити:

**1. Породжувальні (Creational)** — приховують ЩО створюється, КОЛИ та ЯК. Код не повинен залежати від конкретних класів через `new`.
...
**3. Поведінкові (Behavioral)** — вирішують: хто викликає кого, коли та за яких умов.
- Як `Strategy` — заміна алгоритмів
- Як `Observer` — підписка на події
- Як `Iterator` — перебір елементів

**Проста аналогія:** Будуємо будинок
- **Породжувальні** — як створити цеглу, вікна, двері
- **Структурні** — як з'єднати їх разом
- **Поведінкові** — як люди будуть користуватися будинком

### Коли НЕ думати в категоріях категорій

Для простих CRUD-застосунків категорії патернів надлишкові. Використовуйте патерн тільки коли він вирішує конкретну проблему.
...
**Фокус:** Композиція класів у більші структури

**Навіщо:** Вирішення проблем несумісності без успадкування
...
- `Hexagonal Architecture` — порти й адаптери
...
### Архітектурний зміст категорій

**Категорія = рівень управління складністю:**
...
DIP (Dependency Inversion Principle) — принцип інверсії залежностей: залежати від абстракцій, не від деталей.
OCP (Open/Closed Principle) — принцип відкритості/закритості: відкритий для розширення, закритий для модифікації.

```
Якщо в бізнес-коді багато `new` → порушення DIP

Рішення:
  1. Simple Factory (статичний метод)
  2. Factory Method (успадкування)
...
### Structural: Композиція vs Успадкування

**Decorator замість успадкування:**
```java
// ❌ Успадкування → вибух класів
...
Megamorphic call (>2 реалізацій)
  → Непрямий виклик через vtable
  → Вимірна просадка по CPU
```
...
Inventory Service → зарезервувати товар
  ↓ (якщо помилка)
Compensating Transactions → відкотити все
```
...
1. **Визначайте категорію** при виборі патерна
2. **Creational** → боріться з `new` у бізнес-коді
3. **Structural** → композиція > успадкування
...
- **Structural** = управління складністю зв'язків
- **Behavioral** = управління логікою взаємодії
- **Категорія** = рівень управління архітектурною складністю
...
- Категорії допомагають визначати корінь архітектурних проблем і вибирати правильне рішення
...
- **Коли категорії патернів надлишкові?** — У простих CRUD-застосунках і прототипах
...
- ❌ «Структурні патерни — це тільки для GUI» — Adapter, Decorator, Proxy скрізь у Spring

**Пов'язані теми:**
- [Що таке патерни проектування](01.%20Що%20таке%20патерни%20проектування.md) — загальний вступ та рівні патернів
- [Що таке Singleton](03.%20Що%20таке%20Singleton.md) — Creational патерн
- [У чому переваги Decorator перед успадкуванням](12.%20У%20чому%20переваги%20Decorator%20перед%20успадкуванням.md) — Structural патерн
- [Як працює Strategy](10.%20Як%20працює%20Strategy.md) — Behavioral патерн
- [Які типи Proxy існують](14.%20Які%20типи%20Proxy%20існують.md) — Structural патерн у Spring
