...
**Prototype** — патерн, який дозволяє створювати нові об'єкти шляхом копіювання існуючого. Copy constructor — рекомендована Java-альтернатива Object.clone().
...
User copy = User.copyOf(original);  // Зрозуміло та типобезпечно
```
...
public User(User other) {
    this.name = other.name;               // String — імутабельна, ок
...
public class OrderProcessor {
    ...
    public OrderProcessor(Order order) {
        this.order = order;  // Щоразу новий Order
    }
}
...
// ⚠️ Пастка: Prototype в Singleton
@Component  // Singleton!
...
public class OrderService {
    ...
    public void process() {
        OrderProcessor processor = processorProvider.getObject();  // Новий щоразу!
    }
}
```
...
```java
// Для складних об'єктів із глибокою вкладеністю
...
// → Копіює пам'ять, обходячи конструктори
...
- OrderProcessor (prototype) впроваджений в OrderService (singleton)
- Баг: усі замовлення оброблялися одним процесором
...
**Обов'язково знати:**
- Prototype — створення об'єктів копіюванням існуючого, а не через конструктор
- Cloneable в Java — поганий дизайн: маркерний інтерфейс без методу clone(), поверхнева копія, не працює з final
...
- Prototype в Singleton — пастка: створиться один раз, рішення через ObjectProvider або @Lookup
...
- **Чим глибоке копіювання відрізняється від поверхневого?** — Глибоке копіює всі вкладені об'єкти, поверхневе — тільки посилання
- **Що буде якщо Prototype бін впровадити в Singleton?** — Створиться один раз при ініціалізації Singleton
...
- ❌ «Cloneable швидший за конструктор копіювання» — benchmark показує протилежне (80ms vs 50ms)

**Пов'язані теми:**
- [Коли використовувати Builder](08.%20Коли%20використовувати%20Builder.md) — породжувальний патерн
...
- [Що таке Singleton](03.%20Що%20таке%20Singleton.md) — Singleton vs Prototype scopes
- [Які категорії патернів існують](02.%20Які%20категорії%20патернів%20існують.md) — Creational патерни
- [Які антипатерни ви знаєте](16.%20Які%20антипатерни%20ви%20знаєте.md) — Copy-Paste Programming
